...
public class TaskService : ITaskService
{
    ...
    public TaskService(ITaskRepository taskRepository, IProjectService projectService, IWebHostEnvironment environment)
    {
    ...
    public async Task<List<TaskViewModel>?> GetTasksByProjectIdAsync(int projectId)
    {
        List<ToDoList> tasks = await _taskRepository.GetTasksByProjectIdAsync(projectId);
        return tasks.Select(t => new TaskViewModel
        {
            ...
            Priority = t.Priority,
            TeamMembers = GetTeamMembersAsync(projectId).Result.TeamMembers
        }).ToList();
    }
    ...
    public async Task<bool> UpdateTaskAsync(TaskViewModel model, int userId)
    {
        ...
        task.CreatedOn = TimeZoneInfo.ConvertTimeToUtc(model.CreatedOn ?? DateTime.UtcNow, localZone);
        task.DueDate = TimeZoneInfo.ConvertTimeToUtc(model.DueDate ?? DateTime.UtcNow, localZone);
    ...
    public async Task<TaskViewModel> GetTeamMembersAsync(int projectId)
    {
        if (projectId == 0)
        {
        ...
        List<ProjectUser> teamMembers = await _taskRepository.GetTeamMembersAsync(projectId);
        ProjectViewModel? project = await _projectService.GetProjectByIdAsync(projectId);
        TaskViewModel task = new();
        foreach (ProjectUser teamMember in teamMembers)
        {
            task.TeamMembers.Add(new UserViewModel
            {
        ...
        task.ProjectStartDate = project?.StartDate ?? DateTime.Now;
        task.ProjectEndDate = project?.EndDate ?? DateTime.Now;
        return task;
    }

    public async Task<int> AddTaskAsync(TaskViewModel model, UserViewModel user)
    {
        TimeZoneInfo localZone = TimeZoneInfo.FindSystemTimeZoneById("India Standard Time");
        ToDoList task = new()
        {
            ...
            Status = "To Do",
            CreatedOn = TimeZoneInfo.ConvertTimeToUtc(model.CreatedOn ?? DateTime.UtcNow, localZone),
            DueDate = TimeZoneInfo.ConvertTimeToUtc(model.DueDate ?? DateTime.UtcNow, localZone),
            Priority = model.Priority,
            IsDeleted = false
        };

        model.TaskId = await _taskRepository.AddTaskAsync(task);
